#!/usr/bin/env python3
"""Run the frozen M15 binary regression gate with post-analysis oracles."""
...
def parse_arguments() -> argparse.Namespace:
    default_root = Path(__file__).resolve().parent / "m15_blind"
    parser = argparse.ArgumentParser(description="Run the M15 blind binary regression gate")
...
def _load_oracles(path: Path) -> tuple[BlindBenchmarkOracle, ...]:
    ...
    if not isinstance(payload, list):
        raise ValueError("M15 oracle file must contain an array")
